...
struct  GISMAP_API ICalGis_MapManger
{
	...
	enum CalMapType
	{
		...
		CALMAP_CON = 5,
		CALMAP_INDOOR = 6,
		CALMAP_VECTOR_AGL = 7,/*建筑物高度为地面以上，并且mif格式的话，mid文件只有1列（高度）*/
        CALMAP_VECTOR_ASL = 8,/*建筑物高度为海拔以上，并且mif格式的话，mid文件只有1列（高度）*/
		CALMAP_ALL = 9
	};
	...
	virtual short GetDemHeight(double x, double y)=0;			//获取地形图的高度
        virtual short GetDhmHeight(long x, long y)=0;			//获取地面建筑物高度
	
	//判断x,y位置处的值是否是有效值
	virtual bool isInvalidValue(int x, int y)=0;
	//判断给定的点是否在地图外面
