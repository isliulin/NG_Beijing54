...
bool VoronoiDiagramGenerator::generateVoronoi(vector<NGPOINT>& values,double minX, double maxX, double minY, double maxY, double minDist)
{
	cleanup();
	cleanupEdges();
	int i;
	...
	sites = (struct Site *) myalloc(nsites*sizeof( *sites));//申请离散点的内存
	...
	qsort(sites, nsites, sizeof (*sites), scomp);//对所有的点进行排序 ，y、x  从小到大排序
...
struct Halfedge * VoronoiDiagramGenerator::ELleftbnd(struct NGPOINT *p)
{
	int i, bucket;
	struct Halfedge *he;
...
struct Site * VoronoiDiagramGenerator::intersect(struct Halfedge *el1, struct Halfedge *el2, struct NGPOINT *p)
{
	struct	Edge *e1,*e2, *e;
	struct  Halfedge *el;
	double d, xint, yint;
...
int VoronoiDiagramGenerator::right_of(struct Halfedge *el,struct NGPOINT *p)
{
	struct Edge *e;
	struct Site *topsite;
...
struct NGPOINT VoronoiDiagramGenerator::PQ_min()
{
	struct NGPOINT answer;
...
void VoronoiDiagramGenerator::pushGraphEdge(double x1, double y1, double x2, double y2,int leftIndex,int rightIndex)
{
	...
	newEdge->x2 = x2;
	newEdge->y2 = y2;
	newEdge->leftIndex = leftIndex;
	newEdge->rightIndex = rightIndex;
	edgeNum ++;
}
...
void VoronoiDiagramGenerator::line(double x1, double y1, double x2, double y2,int leftIndex,int rightIndex)
{	
	pushGraphEdge(x1,y1,x2,y2,leftIndex,rightIndex);
...
{
	struct NGPOINT *s1 = (NGPOINT*)p1, *s2=(NGPOINT*)p2;
