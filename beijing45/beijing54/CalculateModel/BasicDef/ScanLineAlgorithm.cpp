				  ...
				  bool** ppbTemplate, long yHeight, long xWidth)
{
	...
	dYIndex += dYResolution/2.0; //看栅格中心点的情况
	...
	for( ;dYIndex<edgetable.yMax; dYIndex+= dYResolution)
	{
		...
		if(vIntersectX.size() != 0)
		{
			...
			//针对扫描线上的每点, 四舍五入
			y = (int)((dYIndex - dLowLeftY)/dYResolution);
			if(y>=yHeight) continue;
			for(dXIndex=edgetable.xMin + dXResolution/2.0; dXIndex<edgetable.xMax; dXIndex+=dXResolution)
			{
				if(dXIndex < *iter)
				{
					if(!bOut) 
					{
						//获取该点的相对坐标
						x = (int)((dXIndex - dLowLeftX)/dXResolution);
